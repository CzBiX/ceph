...
void CInode::validate_disk_state(CInode::validated_data *results,
				 Context *fin)
{
  class C_CheckBacktrace : public Context {
  public:
    CInode *in;
    validated_data *results;
    bufferlist bl;
    Context *fin;
    C_CheckBacktrace(CInode *i, validated_data *r, Context *f) :
      in(i), results(r), fin(f) {}
    void finish(int r) {
      in->validate_backtrace(results, bl, fin, r);
    }
  };
  ...
  C_CheckBacktrace *c = new C_CheckBacktrace(this, results, fin);

  fetch_backtrace(&c->bl, new C_OnFinisher(c, &mdcache->mds->finisher));
}

void CInode::validate_backtrace(CInode::validated_data *results,
				bufferlist bl,
				Context *on_finish,
				int rval)
{
  ...
  if (gather.has_subs()) {
    ...
    class C_ValidateDirfrags : public MDSInternalContext {
      CInode *in;
      validated_data *results;
      Context *on_finish;
    public:
      C_ValidateDirfrags(CInode *i, validated_data *r, Context *c,
			 MDS *m) : MDSInternalContext(m),
				   in(i), results(r), on_finish(c) {}
      
      void finish(int rval) {
	in->validate_dirfrags(results, on_finish, rval);
      }
    };
    
    gather.set_finisher(new C_ValidateDirfrags(this, results,
					       on_finish, mdcache->mds));
    gather.activate();
    return;
  }
  ...
  validate_dirfrags(results, on_finish, 0);
}

void CInode::validate_dirfrags(CInode::validated_data *results,
			       Context *on_finish,
			       int rval)
{
  ...
  results->raw_rstats.memory_value = inode.rstat;
